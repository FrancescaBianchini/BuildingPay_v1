...
class BuildingPayConfig(models.Model):
    ...
    contratto_template = fields.Binary(
        string='Template Accordo Condomini Aggregati',
        attachment=True,
        help='File .docx da utilizzare come template per l\'Accordo Condomini Aggregati.',
    )
    contratto_template_filename = fields.Char(
        string='Nome file template',
        default='Accordo Condomini Aggregati.docx',
    )
    ...
    def get_contratto_template_attachment(self):
        """Restituisce l'attachment del template contratto."""
        self.ensure_one()
